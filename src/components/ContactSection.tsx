...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // WhatsApp-Benachrichtigung an den Vermieter wird als Server-Side-Effect
      // innerhalb von send-transactional-email ausgelöst (kein public Endpoint mehr).
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Popover open={startOpen} onOpenChange={(open) => { setStartOpen(open); if (open) refetch(true); }}>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <SeasonCalendar mode="single" locale={dfnsLocale} selected={startDate} onSelect={(date) => { if (date && (date < earliestStart || isDateUnavailable(date, minDays, earliestStart) || isOutOfSeason(date))) return; setStartDate(date); if (date) { const autoEnd = computeAutoEnd(date); setEndDate(autoEnd); } setStartOpen(false); }} defaultMonth={startDate ?? suggestedStart ?? calendarDefaultMonth} fromMonth={startOfMonth(new Date())} disabled={(date) => date < earliestStart || isOutOfSeason(date) || isDateUnavailable(date, minDays, earliestStart)} initialFocus className="p-3 pointer-events-auto" weekStartsOn={1} modifiers={{ booked: (date) => date < earliestStart || isDateUnavailable(date, minDays, earliestStart) || isOutOfSeason(date), suggested: (date) => !startDate && !!suggestedStart && date.getTime() === suggestedStart.getTime() }} modifiersClassNames={{ booked: "rdp-day_booked !bg-destructive/20 !text-destructive !opacity-100 font-semibold ring-1 ring-destructive/30 cursor-not-allowed", suggested: "!bg-primary !text-primary-foreground font-semibold ring-2 ring-primary" }} components={{ DayContent: ({ date }) => renderCalendarDay(date) }} />
                    </PopoverContent>
                  </Popover>
                  <Popover open={endOpen} onOpenChange={(open) => { setEndOpen(open); if (open) refetch(true); }}>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <SeasonCalendar mode="single" locale={dfnsLocale} selected={endDate} onSelect={(date) => { if (date && (isOutOfSeason(date) || (minEndDate && date < minEndDate) || (startDate && isCheckoutBlocked(date, startDate)))) return; setEndDate(date); setEndOpen(false); }} defaultMonth={endDate ?? endCalendarDefaultMonth} fromMonth={startOfMonth(startDate ?? new Date())} disabled={(date) => date < (startDate || new Date()) || isOutOfSeason(date) || (minEndDate ? date < minEndDate : false) || (startDate ? isCheckoutBlocked(date, startDate) : isDateBooked(date))} initialFocus className="p-3 pointer-events-auto" weekStartsOn={1} modifiers={{ booked: (date) => (startDate ? isCheckoutBlocked(date, startDate) : isDateBooked(date)) || isOutOfSeason(date), tooShort: (date) => !!(minEndDate && startDate && date >= startDate && date < minEndDate), suggested: (date) => !endDate && !!suggestedEnd && date.getTime() === suggestedEnd.getTime() }} modifiersClassNames={{ booked: "rdp-day_booked !bg-destructive/20 !text-destructive !opacity-100 font-semibold ring-1 ring-destructive/30 cursor-not-allowed", tooShort: "!bg-destructive/20 !text-destructive !opacity-100 font-semibold ring-1 ring-destructive/30 cursor-not-allowed", suggested: "!bg-primary !text-primary-foreground font-semibold ring-2 ring-primary" }} components={{ DayContent: ({ date }) => renderCalendarDay(date) }} />
                    </PopoverContent>
                  </Popover>
                </div>
                {calendarLoading && (
