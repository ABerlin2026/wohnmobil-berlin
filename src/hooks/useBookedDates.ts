...
// Module-level shared state so multiple components (ContactSection,
// AvailabilityChecker, ...) trigger only ONE network request instead of N
// parallel ones. This also keeps the request out of the critical path.
let sharedRanges: BookingRange[] = [];
let sharedLastFetch = 0;
let sharedInflight: Promise<BookingRange[]> | null = null;
const subscribers = new Set<(r: BookingRange[]) => void>();

function notify(ranges: BookingRange[]) {
  sharedRanges = ranges;
  subscribers.forEach((cb) => cb(ranges));
}

async function doFetch(): Promise<BookingRange[]> {
...
// Schedule a callback once the browser is idle / after the page has loaded,
// so the booking fetch is not part of the LCP critical request chain.
...
export function useBookedDates() {
  const [bookedRanges, setBookedRanges] = useState<BookingRange[]>(sharedRanges);
  const [loading, setLoading] = useState(sharedRanges.length === 0);
  const [error, setError] = useState<string | null>(null);
  const lastFetchRef = useRef<number>(sharedLastFetch);
  const inflightRef = useRef<Promise<void> | null>(null);

  const fetchBookings = useCallback(async (force = false) => {
    const now = Date.now();
    if (!force && now - sharedLastFetch < STALE_MS && sharedRanges.length > 0) {
      setBookedRanges(sharedRanges);
      setLoading(false);
      return;
    }
    setLoading(true);
    try {
      ...
      setError(null);
      lastFetchRef.current = sharedLastFetch;
    } catch (err) {
  ...
  useEffect(() => {
    const cb = (r: BookingRange[]) => setBookedRanges(r);
    subscribers.add(cb);
    ...
    if (sharedRanges.length > 0) {
      ...
      // Defer until idle/after load to keep this request off the
      // critical request chain (improves LCP / network-dependency-tree).
      scheduleIdle(() => {
        fetchBookings(true);
      });
    }
